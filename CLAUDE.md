...
- **26 September 2026, new work on top of all the above:** subscribing is now allowed during the 60-day free trial (removed a client-side refusal, no backend change needed), and a sole active organiser can now delete their own account directly (previously hard-blocked) -- both David's explicit product decisions. New migration `20260926160000_allow_sole_organiser_account_deletion.sql`, not yet applied to any hosted project. Full detail: `docs/REVISION_LOG.md`'s 26 September entry.
