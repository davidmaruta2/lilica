...
- **26 September 2026, new work on top of all the above -- migrated to production and OTA'd, on David's explicit instruction:** subscribing is now allowed during the 60-day free trial; a sole active organiser can now delete their own account directly (previously hard-blocked); a contributor can now leave every care space they belong to, not only the active one. Migration `20260926160000_allow_sole_organiser_account_deletion.sql` applied to both `lilica-development` and `lilica-production`. OTA published to `production`/runtime `1.0.0`/both platforms, update group `fdfb8eb6-1a17-4bbe-82cc-b24c28ea3a3a`, commit `3f979d3`. Full detail, including the schema-push-succeeded-but-full-pgTAP-still-outstanding caveat: `docs/REVISION_LOG.md`'s 26 September entries.
