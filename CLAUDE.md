...
## Apple ASN V2 / Google RTDN: confirmed configured (19 September 2026)

The pending task from earlier in the day (checking Apple Server Notifications V2 / Google RTDN in the RevenueCat dashboard, interrupted by a computer restart) is now closed. Apple: RevenueCat's one-click "Apply to App Store Connect" button succeeded. Google: required granting the RevenueCat service account the Pub/Sub Editor IAM role on Google Cloud project `lilica-6gy1l5` first, then RevenueCat's "Connect to Google" button succeeded. Full detail: `docs/REVISION_LOG.md`'s 19 September entry and `docs/LUMEN_HANDOFF.md`. There is no other pending task queued -- do not invent one; wait for David's next instruction.
