...
## Where things stand right now (verified live 23 September 2026, late evening)

- Installed native binary: iOS/Android **`1.0.0 (6)`**, built from commit `195d186`, `buildProfile: production` on both platforms. Verified via `eas build:list --limit 3 --json` (grep `appBuildVersion`) -- do not trust a build number from prose in this or any doc; always re-check this way. No new native build since 22 September.
- **Store status:** unchanged since 22 September -- Apple `WAITING_FOR_REVIEW` (reviewSubmission `d9d53c6e-d2bb-4ca7-9304-a3afc274d542`, build `1.0.0 (6)`); Google Play production track release `versionCode 6`, status `completed`, live. Re-verify live before repeating either fact (JWT-signed API calls, patterns in `docs/REVISION_LOG.md`).
