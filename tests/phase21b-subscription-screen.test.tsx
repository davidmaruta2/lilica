...
describe('SubscriptionScreen', () => {
  ...
  it('offers Subscribe (not Manage) while not yet a paid subscriber', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({})} loading={false} billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
    screen.getByText('Subscribe for £8.99/year');
  ...
  it('offers Manage subscription (not Subscribe) once actively subscribed', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({ status: 'SUBSCRIPTION_ACTIVE' })} loading={false} billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
  ...
  it('explains expiry without ever implying data was removed', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({ status: 'TRIAL_EXPIRED' })} loading={false} billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
  ...
  it('tapping Subscribe calls onSubscribe and shows an error message on failure', async () => {
    ...
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({})} loading={false} billingConfigured onBack={jest.fn()} onSubscribe={onSubscribe} onRestore={jest.fn()} />,
    );
  ...
  it('tapping Restore purchases calls onRestore and reports the result', async () => {
    ...
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({})} loading={false} billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={onRestore} />,
    );
  ...
  it('disables Subscribe/Restore and explains when billing is not yet configured in this build', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({})} loading={false} billingConfigured={false} onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
  ...
  it('shows a loading state rather than a blank or fabricated status', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={undefined} loading billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
  ...
  it('shows a real error message rather than a silent failure', async () => {
    const screen = await render(
      <SubscriptionScreen entitlement={undefined} loading={false} error="Could not load your subscription." billingConfigured onBack={jest.fn()} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
  ...
  it('Back returns without acting', async () => {
    ...
    const screen = await render(
      <SubscriptionScreen entitlement={entitlement({})} loading={false} billingConfigured onBack={onBack} onSubscribe={jest.fn()} onRestore={jest.fn()} />,
    );
