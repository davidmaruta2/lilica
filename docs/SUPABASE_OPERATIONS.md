...
| Production | Future live user data | `lilica-production` (`luyoyupghbxjftqwzcfn`), Luxford Interactive, eu-west-2 | Created 19 September 2026 on David's explicit instruction; all 22 migrations applied and pgTAP-proven (23 files/496 assertions); `entitlement-webhook`/`send-invitation-email` edge functions deployed; `REVENUECAT_WEBHOOK_SECRET`/`RESEND_API_KEY` secrets set. **Fully configured and independently verified as of 19 September 2026:** RevenueCat production webhook integration exists and is correctly scoped to production-only events (see `docs/REVISION_LOG.md` for a cross-environment webhook bug found and fixed the same day); Auth SMTP, `site_url`/redirect URLs, and `otp_length` all confirmed correct via `supabase config diff`. No production build/data exists yet -- this readiness does not authorise one. **Always re-link the CLI back to `lilica-development` after any production operation** -- `supabase link --project-ref ldocquqbcabdbscghojc` -- so day-to-day work doesn't silently target production. |
