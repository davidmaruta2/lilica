...
## 24. RevenueCat production-readiness preflight (16 September 2026)

This is the repository-only first step of the approved external subscription setup sequence. It does not create products, credentials, builds, webhooks or releases in Apple, Google, RevenueCat, Supabase or EAS.

- **60-day boundary**: Lilica's free period remains backend-owned and starts with the first genuinely new care space. It is not duplicated as a store introductory offer. While `TRIAL_ACTIVE` is still within `trial_expires_at`, the Subscription screen shows that the user will not be charged and exposes no purchase CTA; `handleSubscribe()` repeats the guard so a stale or modified render cannot initiate an early purchase. Restore Purchases remains available.
- **Exact annual product only**: `getAnnualPackage()` now returns only RevenueCat's `PACKAGE_TYPE.ANNUAL`. A missing/misconfigured annual package fails closed instead of falling back to the first available package and potentially selling an unapproved duration.
- **Localized store price**: opening Subscription fetches the current annual package and passes its store-provided `product.priceString` into every purchase/renewal disclosure. Purchase remains disabled if that metadata cannot be loaded. The approved UK store price remains £8.99/year, but no hard-coded GBP value is presented as the purchase price to a user whose store may use another localized currency.
- **Current webhook signature contract**: HMAC verification parses RevenueCat's `t=<unix timestamp>,v1=<hex signature>` header, computes HMAC-SHA256 over `<timestamp>.<raw JSON body>`, compares in constant time, and rejects timestamps outside a five-minute tolerance. Provider event IDs remain the independent database idempotency boundary.
- **Environment contract**: `.env.example` now documents `EXPO_PUBLIC_REVENUECAT_IOS_API_KEY` and `EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY` as public app-specific SDK configuration. RevenueCat secret API keys and `REVENUECAT_WEBHOOK_SECRET` remain server-only and must never use `EXPO_PUBLIC_*` or enter Git.

Validation at this preflight checkpoint: TypeScript clean; 9 focused Jest suites/103 assertions passing; secret scan, public Expo config, web export and `git diff --check` clean. The repository-wide combined Jest command retained its known timeout behaviour, and Expo's compatibility check continues to report the pre-existing SDK 57 patch-level package drift; neither produced a preflight-specific failure. Real platform purchase, restore, renewal, cancellation, refund, expiry and signed webhook delivery remain unverified until the external console and native-build steps are completed.
