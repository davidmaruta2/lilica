...
Do not build production AI, bank integrations, payment initiation, emergency monitoring, fall detection, eMAR, diagnosis, complex family permissions, provider integrations or a full OCR pipeline without explicit approval. Subscriptions are no longer out of scope: **Phase 21 - Billing, Subscription & Membership/Entitlement** is now implemented server-side and client-side (14 September 2026) - one subscription per user account (not per supported person) covers every care space that account commercially owns, kept architecturally separate from Care Circle security membership (`care_space_memberships`). The approved pricing is 60 days fully free, then £8.99/year. **Real purchases/webhooks remain unverified**: no RevenueCat project, App Store/Google Play product, or development build exists yet, so no physical purchase has ever occurred - see `docs/PHASE_21_ARCHITECTURE.md`/`docs/PHASE_21_QA.md`. Phase 20 - Competitive Gap Review & Carer-Focused Enhancements (an audit against established carer products, beginning with Jointly/Carers UK) is complete, with exactly three approved enhancements (Recent Activity, Search, Care Summary) implemented and physically confirmed. **Phase 22 - Premium Visual Optimisation & Experience Polish** has completed its audit and step-by-step product-owner visual review. The direction is approved by the product owner and recorded in `docs/PHASE_22_APPROVED_VISUAL_DIRECTION.md`; no application implementation has started, and GPT review plus a separate explicit implementation prompt remain mandatory.
...
Do not add fake records, fabricated statistics, profile-completion prompts, feature grids or empty dashboard sections. Calendar (Phase 11 of the canonical roadmap) and To Do (Phase 12) are now real projections rather than placeholders. The fourth tab (Phase 13, renamed People by Corrective Task 10) is likewise a real projection, not a placeholder - see `docs/PHASE_13_ARCHITECTURE.md`/`docs/CORRECTIVE_TASK_10_PEOPLE_IA.md`.
