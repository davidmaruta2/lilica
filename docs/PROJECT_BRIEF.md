...
Do not build production AI, bank integrations, payment initiation, emergency monitoring, fall detection, eMAR, diagnosis, complex family permissions, provider integrations or a full OCR pipeline without explicit approval. Subscriptions are no longer indefinitely out of scope: on 13 September 2026 the product owner approved a roadmap extension in which commercial subscription work is explicitly scheduled for **Phase 21 — Billing, Subscription & Membership/Entitlement** (see `docs/CORE_SYSTEM_CONTRACT.md`). It has NOT been implemented yet. The current approved pricing direction is two months fully free, then £8.99/year, as one subscription per user account rather than per supported person; commercial subscription "membership" is kept architecturally separate from Care Circle security membership (`care_space_memberships`). The same 13 September 2026 extension also approved **Phase 20 — Competitive Gap Review & Carer-Focused Enhancements** (an audit against established carer products, beginning with Jointly/Carers UK, with implementation limited to explicitly approved gaps) and **Phase 22 — Premium Visual Optimisation & Experience Polish** (the final planned phase, which must begin with an audit/report only before any implementation). None of Phases 20-22 has started.
