...
11. Complete store listings, privacy/policy declarations, review assets, and explicit production release approval. **Apple side done 20 September 2026:** privacy policy/terms/support pages published at lilica.co.uk; App Store Connect description, keywords, subtitle, promotional text, copyright, category (Lifestyle), age rating declaration, and all 10 store screenshots are set. **Google Play side done 20 September 2026:** 8 store screenshots uploaded and committed via the Android Publisher API. Still outstanding for either store: App Review contact/demo info if reviewers need to sign in, and the actual "submit for review"/publish action, both separate protected release operations. See `docs/REVISION_LOG.md`'s 20 September entries.
