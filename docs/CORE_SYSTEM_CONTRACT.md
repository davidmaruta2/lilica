...
Implementation note, 13 September 2026 (this note supersedes the one above for overall progress -- it is not rewritten, to preserve an honest history): every phase this roadmap defines through **Phase 18 - Privacy, Settings, Export, And Account Lifecycle** (above) is now implemented, automatically validated, and committed to `master`. Phase 18's own account-deletion/export deliverables were split, working title "Phase 18A"/"Phase 18B", across two separate approved briefs (see `docs/PHASE_18_ARCHITECTURE.md`) rather than delivered as one pass -- real account deletion (`delete_my_account()`, membership detachment/former-member snapshotting, not the destructive cascade this document's own "explicit exclusions" line warns against) and complete authorised-document export are both real, not the placeholder this roadmap's Phase 18 section originally anticipated needing further specialist sign-off for. Several of Phase 18's own listed dependencies (specialist legal/retention decisions -- D25/D26 below) were NOT separately obtained; the shipped implementation instead follows the same STOP-and-report discipline this document itself models (`docs/PHASE_18_ARCHITECTURE.md`'s own "Known Limitations" sections name exactly what remains a genuine open legal/product question, e.g. no formal retention-period policy, no legal conclusions asserted). **Phase 19 - Production Hardening And Release Readiness has NOT been started.** Physical-device QA remains outstanding for most phases -- see `docs/LUMEN_HANDOFF.md`'s own per-phase QA status, which is the authoritative current-state summary; this document remains the architectural roadmap, not a live status tracker. Operational limits are in `docs/SUPABASE_OPERATIONS.md`.
...
This is a documentation-only roadmap extension. Phases 1-19 remain the original canonical roadmap; Phases 20-22 are a subsequent, separately-approved extension, not a retroactive rewrite of the original Phase 2 contract. Phase 19 remains the current phase and is unchanged by this note. No Phase 20, 21 or 22 implementation has started.
...
**Regression protection:** Lilica's existing information architecture (Home, Calendar, To Do, People, universal Add, shared underlying records) must be protected. Phase 20 must not turn Lilica into a large feature-directory/menu merely to match a competitor; any approved enhancement should fit naturally into Lilica's existing model where possible.

**Explicit exclusions:** implementing any enhancement before the audit/report is reviewed and approved; assuming any specific competitor capability will be copied; billing/subscription work (Phase 21); visual/experience redesign (Phase 22).

**Stop point:** after the gap report, stop. GPT and the product owner must review and explicitly approve the precise enhancement set before any Phase 20 implementation prompt is issued.
...
**Explicit exclusions:** scaffolding billing during Phase 19 or Phase 20; altering the security membership model to express commercial state; implementing any of the above in this documentation task.

**Stop point:** Phase 21 receives its own full implementation brief after Phase 20 and after the required commercial product decisions above have been reviewed and approved by the product owner. Do not scaffold billing before then.

### Phase 22 - Premium Visual Optimisation And Experience Polish

**Purpose:** the final planned roadmap phase, after the feature/product and commercial model have stabilised. Its purpose is to make the complete product feel premium, deliberate, cohesive, modern, calm, visually distinctive, professionally designed, and consistent across iPhone and Android. This is not permission for an implementation agent to casually redesign the app.
...
PHASE 21 — Billing, Subscription & Membership/Entitlement
     Implement the approved one-month-free -> £9.99/year model,
...
Phase 19 is not to be rewritten or have Phase 20-22 work retroactively folded into it; it remains Production Hardening & Release Readiness and continues to exclude new product functionality. The extension begins after Phase 19.
