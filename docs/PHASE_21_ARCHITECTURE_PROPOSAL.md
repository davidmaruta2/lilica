...
## 9. Exact one-month semantics

**Recommended: exactly 30×24 hours (a fixed 30-day duration) from `trial_started_at`, not "one calendar month."** *Why*: a calendar-month rule is genuinely ambiguous at month-end (the brief's own example — started 31 January, calendar-month expiry has no unambiguous "31 February" — would need an arbitrary fallback rule such as "last day of the following month," which is exactly the kind of ambiguity the brief asks to avoid) and calendar months vary in length (28–31 days), meaning two users starting a day apart could get meaningfully different trial lengths depending purely on which month they started in. A fixed 30×24-hour duration is: predictable (every user gets exactly the same trial length, always); technically robust (`trial_started_at + interval '30 days'`, one deterministic Postgres expression, no calendar-arithmetic edge cases); easy to explain ("your free month" reads naturally even though it is precisely 30 days, not calendar-month-exact — nobody notices or cares about the 1–2 day difference from a true calendar month); and compatible with Apple/Google, whose own introductory-trial periods are themselves expressed in fixed day/week/month units, not calendar-month boundaries — see section 11.

## 10. App-managed vs store-managed trial recommendation

**Recommended: Option A — an app-managed, server-tracked one-month free period, with no store subscription started until the user actively chooses to subscribe at (or before) expiry.** Compared directly against Option B (an immediate annual store subscription with a store-provided one-month introductory free trial):

| | Option A: app-managed free month | Option B: store-managed intro trial |
|---|---|---|
| User friction to START using Lilica | None — no payment details required at all | Must enter payment details / confirm a subscription up front, even though nothing is charged for a month |
...
| Restore/cross-platform behaviour | Simpler — nothing to restore until an actual purchase exists; the free month itself is purely an account-level fact, not a store transaction | A trial-in-progress technically has a receipt/transaction to restore, adding an extra state to every restore-flow branch for no real user benefit |
...
|---|---|---|---|---|---|---|---|
| `TRIAL_ACTIVE` | Within 30 days of `trial_started_at`, no purchase yet | Yes | Yes | Yes | Yes (their own space's owner is in trial) | Yes (can purchase early) | Cached, works normally within a reasonable re-verification window |
| `TRIAL_EXPIRED` | 30 days elapsed, no purchase | No (read-only) | Yes | Yes | No, for spaces owned by this account | Yes | Cached "expired" state persists; does not silently re-grant access |
...
2. **Day 1 trial**: `TRIAL_ACTIVE`, full access, no communication beyond a quiet "30 days left" note near Account (section 21 of the brief — not a full-screen interruption).
3. **Final trial day**: still `TRIAL_ACTIVE`, full access; a restrained "your free month ends tomorrow" note.
...
- **A. First month**: no interruption beyond a quiet, discoverable "X days left in your free month" note near Account/Settings — never dominating Home, never a countdown badge on every screen.
...
- **C. Expired/read-only**: a calm, honest one-time explanation ("Your free month has ended. Your information is still here. Subscribe for £8.99/year to continue adding and managing care.") shown when a gated mutation is actually attempted (not pre-emptively on every screen load), plus the same quiet Account-area note now reading "Subscribe to continue managing care."
...
- **J. Subscription management**: reachable from Account/Settings at any time, regardless of current state — shows current state in plain language ("Free month — 12 days left" / "Subscribed, renews 14 September 2027" / "Your subscription has ended") plus Restore and a link to platform management.
