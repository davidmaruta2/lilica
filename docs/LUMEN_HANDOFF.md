...
- Privacy, Settings, Export & Account Lifecycle (Phase 18), 12 September 2026: a new "Privacy & data" Settings entry — real display-name editing, a real data-export RPC written to a local file and shared via the OS share sheet (never stored server-side), "Clear data from this device" (warns of pending work, then signs out), "Leave [care space]" for non-organiser roles, and a "Delete account" entry offering only a real read-only precheck — full account/auth-identity deletion is explicitly NOT implemented, a genuine schema blocker (`care_space_memberships.user_id references auth.users(id) on delete restrict`, itself `on delete restrict`-referenced from every historically-attributed table) rather than an oversight. Also closed a real Phase 17-acknowledged gap with a durable document-cleanup retry queue. Implemented and validated, **NOT committed/pushed** — awaiting product-owner review and physical-device QA. See `docs/PHASE_18_ARCHITECTURE.md`/`docs/PHASE_18_QA.md`.

Phase 9, Calendar, Phase 12, Phase 13, Phase 14, Phase 15, Phase 18 physical-device QA are outstanding; Phase 16/17 physical QA is confirmed passed.
