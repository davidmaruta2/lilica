...
**Phase 16 (Document Maturity — foundation) is implemented and validated, migration dry-run confirmed but not yet applied, as of 12 September 2026** (see the Revision Log entry, and `docs/PHASE_16_ARCHITECTURE.md` for full detail). Real persisted, RLS-protected typed record links (`record_links`) and durable cloud-side attachment metadata plus a private Storage bucket (`record_attachments`) now exist, both gated by the exact same domain-grant decision every record read/write already uses. The client library (`src/recordLinks.ts`, `src/attachments.ts`) is written and tested but not yet called from any screen — RecordEditor/RecordDetail UI integration (the "Related to" picker, "Does anything need doing?" flow, bidirectional detail sections, "View document") is explicitly **Phase 17**, not started. A real pre-existing bug was also fixed along the way: synced attachment metadata never reached a pulled record on a second device (`src/recordSync.ts`'s `localRecordFromRow()`). **Do not begin Phase 17 without its own separate, explicit, bounded implementation prompt.**
