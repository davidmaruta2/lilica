...
**Phase 16 (candidate): Documents — link to a record and/or a real task.** A bounded, investigation-only architecture trace (12 September 2026, `docs/PHASE_16_DOCUMENTS_INVESTIGATION.md`) established what already supports this direction before any implementation brief exists — read that document first. In short: the task-creation pipeline needs no new machinery; a typed `RecordLink` shape already exists (`src/domain/types.ts`) but was never persisted or wired to the real record model, so LINK is genuinely new work; a real, pre-existing gap was found where synced attachment metadata is never read back into a pulled record on a second device (`src/recordSync.ts`'s `localRecordFromRow()`); no document viewer exists anywhere. A companion verification (`docs/PHASE_15_ARCHITECTURE.md`'s 12 September 2026 addendum) confirmed Phase 15's server-side domain-grant enforcement has no gap and needs no fix before this work begins. **Do not begin Phase 16 without its own separate, explicit, bounded implementation prompt** — the investigation above is not that authorisation.
...
Preserve Welcome, privacy gating/version/timestamp, organiser/supported-person separation, local account isolation, multi-person care-space separation, category/multi-record behavior, wheel selectors, record-sheet gestures, real-record-only Home, the current Home presentation, Calendar, To Do, People (renamed from Person by Corrective Task 10), the view/edit separation (`RecordDetail`/`RecordQuickEditor`), the Done/backdrop/swipe-saves-pending-content behaviour, the Calendar/To Do/People visual pass (Home deliberately excluded, reverted to plain), and the Phase 9/Phase 12/Phase 13/Phase 14/Phase 15 everyday-copy/Assigned-to/To-Do/Person/reminder/care-circle scope described above.

Deferred: push/email delivery of invitations, an "assignee no longer has access" UI treatment, in-app role-change/organiser-handoff controls, server/push notification delivery and device registration, cloud document storage/OCR, search/history, production Ask Lilica, production infrastructure, backup guarantees, account/care-space deletion policy, and Phase 16's document link/task/attachment-round-trip work (investigated but not started — see `docs/PHASE_16_DOCUMENTS_INVESTIGATION.md`).
