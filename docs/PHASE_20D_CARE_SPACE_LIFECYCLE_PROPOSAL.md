# Phase 20D - Supported-Person / Care-Space Lifecycle

**STATUS UPDATE, 15 September 2026: the decisions below were reviewed by GPT and the product owner and APPROVED for implementation via `\downloads\20-22.txt`. ARCHIVE (a new, reversible, non-destructive `care_spaces.status`), the tightened multi-organiser DELETE consent model (section 24's own recommendation), and organiser HANDOFF (`promote_to_organiser()`) are now genuinely implemented, tested (396 pgTAP assertions) and deployed to `lilica-development` - see `docs/REVISION_LOG.md`'s "15 September 2026 - Phase 20D structural closure" entry for full implementation detail. The rest of this document is preserved EXACTLY as originally written (below), since it remains the accurate record of the analysis that led to those approved decisions - do not read the "PROPOSAL ONLY" framing immediately below as still current.**

**CLOSURE CORRECTION, 15 September 2026 (same day): the structural-closure completion report initially left the subscription disclosure (section 29 of the original brief - "removing a care space does NOT cancel an App Store/Google Play subscription") out of the new multi-organiser deletion-request/approval journey, deferring it as "worth adding in a future pass." That was corrected the same day, not deferred: `RemoveCareSpaceConfirm.tsx` now shows the disclosure in BOTH the sole-organiser and multi-organiser confirmation (a new `requiresAllOrganisers` prop distinguishes the wording - "Removing does not..." vs "Requesting removal does not..."), and the pending-approval card in `ManageCareScreen.tsx` shows its own calm, permanence-plus-disclosure line to the organiser being asked to approve. This correction also found, by direct code inspection, that the ORIGINAL sole-organiser Remove confirmation never actually carried this disclosure either (the completion report's claim that it was "already correct and already shipped" was mistaken) - both paths now carry it for the first time. See `docs/REVISION_LOG.md`'s matching closure-correction entry.**

---

**Original status (superseded above): PROPOSAL ONLY. Per `prephase22.txt`'s own Part B authority level, no lifecycle RPC, migration, archive/delete mechanism, or destructive button was implemented in producing this document.**
