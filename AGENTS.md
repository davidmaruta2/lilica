...
- To Do (Phase 12 — actionable-work projection: open task/bill/home-or-car-matter records grouped into Overdue/Today/Upcoming, with Mine/Unassigned/All assignment filters on Phase 9's stable membership identity) is implemented and validated; physical-device QA is outstanding — see `docs/PHASE_12_QA.md`. Do not begin Phase 13+ without its own separate, explicit, bounded implementation prompt.
...
Phase 8 is implemented, committed and physically approved. Phase 9 and Calendar (canonical Phase 11, committed under the working label "Phase 10" — see the roadmap numbering note above) are implemented, validated and committed, awaiting physical QA. Phase 12 (To Do) is implemented and validated, awaiting physical QA (`docs/PHASE_12_QA.md`) and product-owner commit approval. Do not begin Phase 13+ without its own separate, explicit, bounded implementation prompt.
