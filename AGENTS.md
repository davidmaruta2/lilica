...
- **Phase 16 (candidate, not started): Documents — link to a record and/or a real task.** A bounded, investigation-only architecture trace (12 September 2026, `docs/PHASE_16_DOCUMENTS_INVESTIGATION.md`) established what already supports this before any implementation brief exists: the task-creation pipeline is fully reusable as-is; a typed `RecordLink` shape exists (`src/domain/types.ts`) but was never persisted/wired up, so LINK is genuinely new work; a real pre-existing gap was found where synced attachment metadata never round-trips back into a pulled record on a second device (`src/recordSync.ts`); no document viewer exists anywhere. Do not begin Phase 16 without its own separate, explicit, bounded implementation prompt — this investigation is not that authorisation.
